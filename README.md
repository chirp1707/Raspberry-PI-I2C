...
4. Go to www.github.com and search for “Raspberry-PI-I2C-Arduino”, then download this as a zip file. Using the file manager go the download directory and unzip the file. You probably should move the unzip directory into your home directory.
5. Program the Arduino ( Leonardo )by going to Programming then Arduino IDE. Once the IDE opens, in the IDE click on File, then Open and go to the unzipped download file that the extension is “ino”, click open.
6. In the upper left corner is a check mark, click on it. This compiles and verifies the Arduino file.
7. Click on Tools and select Boards. Choose the Arduino that is being used. For this example, it will be Arduino Leonardo.
8. Click on Tools and select Serial Port, /dev/ACM0.
9. In the upper left corner is an arrow that points to the right. Click on the this and this should program the Arduino. If everything goes correctly, it will “Done uploading.
10. Using one of the many diagrams of the Raspberry GPIO, connect the SDA, SCL and ground between the Raspberry PI and the Arduino. The Arduino pins are labeled.
11. Run the command “i2cdetect –y 1”. You should get a display of the i2c connections.
12. The input voltage to be measured should be connected to pin A0 on the Arduino. In order for the Raspberry PI software to align the bytes correctly, pin A3 needs to be connected to A3.
13. Run the program by entering “python Rasp_I2C_Ard_20.py”
